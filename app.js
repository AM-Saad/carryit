...
io.of("/tracking").on("connection", function (socket) {
  ...
  socket.on("move", (data) => {
    ...
    if (socket.connected && data) {
      socket.volatile.broadcast
        .to(data.id)
        .emit("move", { coords: data.coords, id: data.id });
    }
  });
  ...
  socket.on("watch", (roomId, cb) => {
    ...
    cb({
      ready:
        rooms.includes(roomId) && activeTrips.includes(roomId) ? true : false,
    });
  });
...
io.of("/chat").on("connection", function (socket) {
    ...
    store.get(sessionID, async function (err, session) {
      if (session.user) {
        ...
        for (const chat of chats) {
          ...
          if (
            session.user._id.toString() === chat.driver.toString() ||
            session.user._id.toString() === chat.admin.toString()
          ) {
  ...
  socket.on("start-upload", function (data) {
    ...
    fs.open(__dirname + "/temp/" + name, "a", 0o755, function (err, fd) {
      if (err) {
        console.log(err);
      } else {
      ...
      fs.write(
        ...
        function (err, Writen) {
          ...
          inp.on("end", function () {
            ...
            socket.emit("Done", {
              Image: "chat-images/" + Name + ".jpg",
              path: "chat-images/" + Name,
            });
          });
        }
      );
...
app.use((error, req, res, next) => {
  return res
    .status(500)
    .json({ message: error.message, status: "UNEXPECTED_ERROR" });
});
